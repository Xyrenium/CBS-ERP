...
export function ProductManagement() {
  ...
  const [schema, setSchema] = useState({ id: '', name: '', category: 'Pipa hitam seamless', stock: 0, minStock: 0, buyPrice: 0, sellPrice: 0 });

  const handleAdd = (e: React.FormEvent) => {
      e.preventDefault();
      if(!schema.id || !schema.name) return;
      setProducts([...products, schema]);
      setSchema({ id: '', name: '', category: 'Pipa hitam seamless', stock: 0, minStock: 0, buyPrice: 0, sellPrice: 0 });
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-white/60 text-[10px] text-indigo-700 uppercase font-bold border-b border-white/50">
                  <tr>
                    <th className="text-left px-6 py-3">SKU/ID</th>
                    <th className="text-left px-6 py-3">Nama Produk & Kategori</th>
                ...
                <tbody className="text-xs text-slate-600 divide-y divide-white/40">
                  {filteredProducts.map(p => (
                    <tr key={p.id} className="hover:bg-white/40 transition-colors">
                      <td className="px-6 py-3 font-medium text-slate-800">{p.id}</td>
                      <td className="px-6 py-3">
                          <div className="font-bold text-emerald-700 glow-text-purple">{p.name}</div>
                          <div className="text-[10px] text-slate-400 mt-0.5">{p.category}</div>
                      </td>
                      <td className="px-6 py-3 text-right">
                          <span className={`font-bold px-2 py-0.5 rounded-full text-[10px] glow-border ${p.stock <= p.minStock ? 'bg-red-500/20 text-red-400 drop-shadow-[0_0_5px_rgba(239,68,68,0.8)]' : 'bg-indigo-100/50 text-indigo-700'}`}>
                              {p.stock}
                          </span>
                      </td>
                      <td className="px-6 py-3 text-right font-medium">Rp {p.sellPrice.toLocaleString('id-ID')}</td>
                      <td className="px-6 py-3 flex justify-end gap-2 items-center h-full pt-4">
                        {role === 'Admin' && (
                          <button onClick={() => handleDelete(p.id)} className="text-red-400 hover:text-red-300 text-[10px] font-bold uppercase transition-colors">
                            HAPUS
                          </button>
                        )}
                      </td>
          ...
          <div className="lg:col-span-4 glow-card rounded-xl p-6 h-fit">
              ...
              <form onSubmit={handleAdd} className="space-y-4">
                  <div>
                      <label className="block text-[10px] font-bold text-indigo-700 uppercase tracking-wide mb-1">SKU / ID Produk</label>
                      <input type="text" required value={schema.id} onChange={e => setSchema({...schema, id: e.target.value})} className="w-full px-3 py-2 glow-input rounded-md text-xs" placeholder="e.g. P004"/>
                  </div>
                  <div>
                      <label className="block text-[10px] font-bold text-indigo-700 uppercase tracking-wide mb-1">Nama Produk</label>
                      <input type="text" required value={schema.name} onChange={e => setSchema({...schema, name: e.target.value})} className="w-full px-3 py-2 glow-input rounded-md text-xs" placeholder="Nama barang..."/>
                  </div>
                  <div>
                      <label className="block text-[10px] font-bold text-indigo-700 uppercase tracking-wide mb-1">Kategori</label>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                      <div>
                          <label className="block text-[10px] font-bold text-indigo-700 uppercase tracking-wide mb-1">Harga Beli</label>
                          <input type="number" required min="0" value={schema.buyPrice} onChange={e => setSchema({...schema, buyPrice: parseInt(e.target.value) || 0})} className="w-full px-3 py-2 glow-input rounded-md text-xs"/>
                      </div>
                      <div>
                          <label className="block text-[10px] font-bold text-indigo-700 uppercase tracking-wide mb-1">Harga Jual</label>
                          <input type="number" required min="0" value={schema.sellPrice} onChange={e => setSchema({...schema, sellPrice: parseInt(e.target.value) || 0})} className="w-full px-3 py-2 glow-input rounded-md text-xs"/>
                      </div>
                  </div>
                  <button type="submit" className="w-full glow-button py-2 rounded-md text-xs font-bold mt-4 uppercase tracking-wider">Simpan Produk</button>
              </form>
          </div>
      </div>
    </div>
