...
export function DocumentPrintView({ sale, type, onBack }: { sale: Sale, type: string, onBack: () => void }) {
    ...
    return (
        <div className="min-h-screen p-4 md:p-8 animate-in fade-in max-w-4xl mx-auto relative">
            ...
            <div ref={componentRef} className="glow-card w-full print-area p-10 mt-4 rounded-xl text-slate-600 text-sm border border-white/50 relative bg-slate-50">
                ...
                {/* Header */}
                <div className="flex justify-between items-start border-b-2 border-slate-300 pb-6 mb-6 gap-4">
                    ...
                    <div className="text-right z-10 relative">
                        <h2 className="text-2xl font-bold uppercase tracking-wider mb-2 text-emerald-700 glow-text drop-shadow-[0_0_5px_rgba(4,192,202,0.4)]">{getTitle()}</h2>
                        <div className="inline-grid grid-cols-2 gap-x-6 gap-y-2 text-xs glow-border p-3 rounded bg-white/60 text-left w-64 backdrop-blur-sm">
                            <span className="font-bold text-indigo-700 uppercase text-[10px]">No. Dokumen</span>
                            <span className="font-bold text-slate-800 text-right drop-shadow-[0_0_3px_rgba(255,255,255,0.5)]">{sale.id}</span>
                            <span className="font-bold text-indigo-700 uppercase text-[10px]">Tanggal Cetak</span>
                            <span className="text-right font-medium text-slate-800">{new Date(sale.date).toLocaleDateString('id-ID', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
                            {(type === 'invoice' || type === 'receipt') && (
                                <>
                                    <span className="font-bold text-indigo-700 uppercase text-[10px]">Term Payment</span>
                                    <span className="text-right font-bold text-emerald-700">{sale.topDays ? `${sale.topDays} HARI` : 'CASH / LUNAS'}</span>
                                </>
                            )}
                        </div>
                    </div>
                ...
                {/* Info Ke / Dari */}
                <div className="mb-8 p-5 bg-white/60 rounded-lg glow-border backdrop-blur-sm relative z-10">
                ...
                {/* Tabel Barang (Kecuali Kwitansi) */}
                {type !== 'receipt' && (
                    <div className="relative z-10 glow-border rounded-lg overflow-hidden mb-8">
                ...
                {/* Footer Signatures */}
                <div className="mt-16 flex justify-between px-10 text-center text-xs relative z-10">
                    <div>
                        <p className="mb-24 font-bold text-slate-400">Hormat Kami,<br/><span className="text-[10px] uppercase tracking-widest text-indigo-700/70 mt-1 block">PT. Caraca Bintang Samudra</span></p>
                    ...
                    {type === 'bast' && (
                        <div>
                            <p className="mb-24 font-bold text-slate-400 leading-relaxed max-w-[200px] mx-auto text-center">Diterima Diatas Kapal Dalam Keadaan Baik Oleh:</p>
                    ...
                    {type === 'do' && (
                        <div>
                            <p className="mb-24 font-bold text-slate-400">Penerima Barang Dokumen,</p>
                            <div className="border-t border-white/50 w-48 mx-auto pt-2">
